/**
 * One-off: merge note text files + existing ORIGINAL/EXTRA
 * -> dedupe CJK-only -> shuffle -> stdout JSON array line
 */
import fs from 'fs';
...
const htmlPath = path.join(rootDir, 'creation-cabin.html');
...
const pasteRaw = fs.readdirSync(notesDir)
